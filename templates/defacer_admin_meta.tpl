...
<div style="background-image: url(<{$xoops_url}>/modules/defacer/assets/images/pages_big.png);
  ...
  <{$smarty.const._AM_DEFACER_METAMAN}>
</div>
<br />
-->
<br style="clear: right; line-height:1px;" />
  <table width="100%" cellspacing="1" cellpadding="0" class="outer">
    <tr align="center">
      ...
      <th><{$smarty.const._AM_DEFACER_META_SITENAME}></th>
      <th><{$smarty.const._AM_DEFACER_META_PAGETITLE}></th>
      <th><{$smarty.const._AM_DEFACER_META_SLOGAN}></th>
      <th><{$smarty.const._AM_DEFACER_META_KEYWORDS}></th>
      <th><{$smarty.const._AM_DEFACER_META_DESCRIPTION}></th>
    ...
    <{foreach item=item key=key from=$items}>
      <tr class="<{cycle values="even,odd"}>" align="center">
        ...
        <td align="left"><{$item.meta_title}></td><!--<{$item.module}><{$item.meta_url}> -->
        <td align="left"><{$item.meta_sitename}></td>
        <td align="left"><{$item.meta_pagetitle}></td>
        <td align="left"><{$item.meta_slogan}></td>
        <td align="left"><{$item.meta_keywords}></td>
        <td align="left"><{$item.meta_description}></td>
        <td>
          <{if $item.meta_vurl != '0'}>
    ...
    <{/foreach}>
    <{else}>
      <tr><td class="head" colspan="9" align="center"><{$smarty.const._AM_DEFACER_NOTFOUND}></td></tr>
    <{/if}>
    <tr>
