...
function defacer_adminMenu($currentoption = 0, $breadcrumb = '')
{
    ...
    if (file_exists($filename = XOOPS_ROOT_PATH . '/modules/' . $defacer->getModule()->dirname() . '/language/' . $GLOBALS['xoopsConfig']['language'] . '/modinfo.php')) {
        ...
        require_once XOOPS_ROOT_PATH . '/modules/' . $defacer->getModule()->dirname() . '/language/english/modinfo.php';
    }
    ...
    echo '<td style="width: 40%; font-size: 10px; text-align: right; color: #2F5376; padding: 0 6px; line-height: 18px;"><br>' . $defacer->getModule()->name() . '  ' . _AM_DEFACER_MODULEADMIN . '</br> ' . $breadcrumb . '</td>';
    echo '</tr></table>';
    echo '</div>';
