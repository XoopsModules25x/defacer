      ...
      <tr class="<{cycle values="even,odd"}>" align="center">
        ...
        <td>
          <{if $item.theme_vurl != '0'}>
            <a href="<{$item.theme_vurl}>" rel="external"><img src="<{xoModuleIcons16 search.png}>" title="<{$smarty.const._PREVIEW}>" alt="<{$smarty.const._PREVIEW}>" /></a>
          <{else}>
            <img src="<{$xoops_url}>/modules/defacer/assets/images/view_big_off.png" title="<{$smarty.const._PREVIEW}>" alt="<{$smarty.const._PREVIEW}>" />
          <{/if}>
      	  <a href="admin_theme.php?op=edit&itemid=<{$item.theme_id}>"><img src="<{xoModuleIcons16 edit.png}>" title="<{$smarty.const._EDIT}>" alt="<{$smarty.const._EDIT}>" /></a>
      	  <a href="admin_theme.php?op=del&itemid=<{$item.theme_id}>"><img src="<{xoModuleIcons16 delete.png}>" title="<{$smarty.const._DELETE}>" alt="<{$smarty.const._DELETE}>" /></a>
        </td>
      </tr>
    <{/foreach}>
    <{else}>
